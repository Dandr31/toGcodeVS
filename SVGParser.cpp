...
namespace SVGParser {
	...
	string SVGSmoothCubicCurveTo::toString2(){
		stringstream s;
		s << "Smooth cubic curve to (" << (absolute?"absolute":"relative") << ") control DPoint: " << x1 << "," << y1 << " to " << x << "x" << y;
		return s.str();
	}
	...
	string SVGQuadraticCurveTo::toString2(){
		stringstream s;
		s << "quadratic curve (" << (absolute?"absolute":"relative") << ") control DPoint: " << x0 << "," << y0 << " to " << x << "x" << y;
		return s.str();
	}
	...
	SVGPath toAbs(SVGPath path) {
		...
		for (SVGPath::iterator it = path.begin(); it != path.end(); it++) {
			...
			case SVG_CUBIC_CURVE_TO_ABS:
				newPath.push_back(new SVGCubicCurveTo(
					((SVGCubicCurveTo*)cmd)->x0, ((SVGCubicCurveTo*)cmd)->y0,
					((SVGCubicCurveTo*)cmd)->x1, ((SVGCubicCurveTo*)cmd)->y1,
					cmd->x, cmd->y, true));
				x = cmd->x;
				y = cmd->y;
				break;
			...
			default: //is is a absolute command, update the coords
				...
				else if (cmd->getType() == SVG_MOVE_TO_ABS) {
					x = cmd->x;
					y = cmd->y;
				}
				newPath.push_back(cmd);
